...
print(f'There are a total of {numberOfServers} servers; all servers are assigned customers from a single queue.\n')
print(f'The next customer arrives between {IAT[0]} and {IAT[1]} time units after the previous customer;\ncustomers wait in line and are assigned to an idle server as one becomes available.\n')
...
while customersServed < customerCount:
    ...
    while (nextCustomer.arrivalTime == clock) and (customersArrived < customerCount):
        ...
        if not queue.isEmpty():
            ...
            allServersBusy = True   # assume true, but check:
            ...
            while s < len(servers):
                if servers[s].customer is None:
                    ...
                    if nextCustomer.serviceDuration > 0:
                        servers[s].customer = nextCustomer
                    else:
                        # customer that just arrived has a zero serviceDuration time, in which case ...
                        # >>> they are immediately served
                        # >>> they did not wait in the queue, so no need to increment totalTimeInQ
                        # >>> their serviceDuration is 0, so no need to increment totalSvcDuration
                        customersServed += 1

                    break   # break out of server loop if an idle server is found

                s += 1

            # if all the servers were busy, then customer must wait in line:
            if allServersBusy:
                queue.enqueue(nextCustomer)
    ...
    while s < len(servers):
        ...
        if servers[s].customer is None:
            '''
            # if this server is available see if there are any customers waiting in its queue
            if not queues[s].isEmpty():
                nextInQ = queues[s].dequeue()
            '''
            # if this server is available see if there are any customers waiting in the queue
            if not queue.isEmpty():
                nextInQ = queue.dequeue()
